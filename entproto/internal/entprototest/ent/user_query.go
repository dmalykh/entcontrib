...
// WithProfilePic tells the query-builder to eager-load the nodes that are connected to
// the "profile_pic" edge. The optional arguments are used to configure the query builder of the edge.
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withProfilePic; query != nil {
		if err := uq.loadProfilePic(ctx, query, nodes, nil,
			func(n *User, e *Image) { n.Edges.ProfilePic = e }); err != nil {
			return nil, err
		}
	}
...
func (uq *UserQuery) loadProfilePic(ctx context.Context, query *ImageQuery, nodes []*User, init func(*User), assign func(*User, *Image)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_profile_pic" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
