...
func (pq *PetQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Pet, error) {
	...
	if query := pq.withAttachment; query != nil {
		if err := pq.loadAttachment(ctx, query, nodes,
			func(n *Pet) { n.Edges.Attachment = []*Attachment{} },
			func(n *Pet, e *Attachment) { n.Edges.Attachment = append(n.Edges.Attachment, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PetQuery) loadOwner(ctx context.Context, query *UserQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *User)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_pet" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (pq *PetQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*Pet, init func(*Pet), assign func(*Pet, *Attachment)) error {
	...
	for _, n := range neighbors {
		fk := n.pet_attachment
		if fk == nil {
			return fmt.Errorf(`foreign-key "pet_attachment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
