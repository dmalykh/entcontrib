...
func (uq *UserQuery) QuerySkipEdge() *SkipEdgeExampleQuery {
	query := (&SkipEdgeExampleClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withAttachment; query != nil {
		if err := uq.loadAttachment(ctx, query, nodes, nil,
			func(n *User, e *Attachment) { n.Edges.Attachment = e }); err != nil {
			return nil, err
		}
	}
	if query := uq.withReceived1; query != nil {
		if err := uq.loadReceived1(ctx, query, nodes,
			func(n *User) { n.Edges.Received1 = []*Attachment{} },
			func(n *User, e *Attachment) { n.Edges.Received1 = append(n.Edges.Received1, e) }); err != nil {
			return nil, err
		}
	}
	if query := uq.withPet; query != nil {
		if err := uq.loadPet(ctx, query, nodes, nil,
			func(n *User, e *Pet) { n.Edges.Pet = e }); err != nil {
			return nil, err
		}
	}
	if query := uq.withSkipEdge; query != nil {
		if err := uq.loadSkipEdge(ctx, query, nodes, nil,
			func(n *User, e *SkipEdgeExample) { n.Edges.SkipEdge = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadGroup(ctx context.Context, query *GroupQuery, nodes []*User, init func(*User), assign func(*User, *Group)) error {
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "user_group" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (uq *UserQuery) loadAttachment(ctx context.Context, query *AttachmentQuery, nodes []*User, init func(*User), assign func(*User, *Attachment)) error {
	...
	for _, n := range neighbors {
		...
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_attachment" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadReceived1(ctx context.Context, query *AttachmentQuery, nodes []*User, init func(*User), assign func(*User, *Attachment)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "received_1" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (uq *UserQuery) loadPet(ctx context.Context, query *PetQuery, nodes []*User, init func(*User), assign func(*User, *Pet)) error {
	...
	for _, n := range neighbors {
		fk := n.user_pet
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_pet" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
...
func (uq *UserQuery) loadSkipEdge(ctx context.Context, query *SkipEdgeExampleQuery, nodes []*User, init func(*User), assign func(*User, *SkipEdgeExample)) error {
	...
	for _, n := range neighbors {
		fk := n.user_skip_edge
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_skip_edge" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
